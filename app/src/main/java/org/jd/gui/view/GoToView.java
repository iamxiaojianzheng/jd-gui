...
public class GoToView {
    ...
    public GoToView(Configuration configuration, JFrame mainFrame) {
        ...
        SwingUtil.invokeLater(() -> {
            goToDialog = new JDialog(mainFrame, MessageUtil.getMessage("navigation.gotoline"), false);
            ...
            hbox = Box.createHorizontalBox();
            hbox.add(Box.createHorizontalGlue());
            JButton goToOkButton = new JButton("   " + MessageUtil.getMessage("navigation.gotoline.ok") + "   ");
            ...
            JButton goToCancelButton = new JButton(MessageUtil.getMessage("navigation.gotoline.cancel"));
                    ...
                    if (text.length() == 0) {
                        ...
                        try {
                            ...
                            if (lineNumber > navigator.getMaximumLineNumber()) {
                                goToOkButton.setEnabled(false);
                                showErrorMessage(MessageUtil.getMessage("navigation.gotoline.error.1"));
                            } else if (navigator.checkLineNumber(lineNumber)) {
                                goToOkButton.setEnabled(true);
                                clearErrorMessage();
                            } else {
                                goToOkButton.setEnabled(false);
                                showErrorMessage(MessageUtil.getMessage("navigation.gotoline.error.2"));
                            }
                        } catch (NumberFormatException e) {
                            goToOkButton.setEnabled(false);
                            showErrorMessage(MessageUtil.getMessage("navigation.gotoline.error.3"));
                        }
                    }
                }
    ...
    public void show(LineNumberNavigable navigator, IntConsumer okCallback) {
        ...
        SwingUtil.invokeLater(() -> {
            // Init
            goToEnterLineNumberLabel.setText(String.format(MessageUtil.getMessage("navigation.gotoline.info"), navigator.getMaximumLineNumber()));
